...
export class AccountService {
  private currentUserSubject = new BehaviorSubject<UserWAdmin>(
    {} as UserWAdmin
  );
  ...
  Login(loginData: Login): Observable<boolean> {
    return this.http
      .post<boolean>(
        'https://antrahrm.azure-api.net/authentication/api/Account/login',
        loginData
      )
      .pipe(
        map((response: any) => {
  ...
  Register(registerData: Register): Observable<boolean> {
    return this.http.post<boolean>(
      'https://antrahrm.azure-api.net/authentication/api/Account/register',
      registerData
    );
  }
